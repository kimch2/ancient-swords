...
function npcAct1()
	...
	elseif(Hero:ActFun(1080,"task has 0",34) == true)then				--判断任务链
		...
		if(Hero:ActFun(1080,"step == 7",34) == true)then				--判断任务步骤
		

	  	if(Hero:ActFun(1080,"task state 0",34) == true)then				--判断任务状态
			
			
			 Hero:ActFun(1080,"task accept 7",34)				--设置任务状态为1
			else
...
function npcAct2()
	...
	elseif(Hero:ActFun(1080,"task has 0",34) == true)then				--判断任务链
	    ...
	    --判断任务是否已完成，已完成则进入下一步
	   	--elseif(Hero:ActFun(1080,"task state 1",34) == true)then				--判断任务状态
	     Hero:ActFun(127,"任务已经完成了",0)				--显示系统文字

	  	elseif(Hero:ActFun(1080,"task state 3",34) == true)then				--判断任务状态
	     Hero:ActFun(1080,"task end 7",34)				--设置第一步状态为完成
	     --Hero:ActFun(1080,"step += 1",34)				--设置任务步骤数加1
	     Hero:ActFun(1080,"task delete 7",34)			--删除任务
	     Hero:ActFun(1080,"mask += 34",34)				--任务结束，加掩码
	     Hero:ActFun(1001,"exp += 7000",0)			--给经验
	     --Hero:ActFun(1001,"petexp += 3500",0)			--给宠物经验
	     Hero:ActFun(1001,"money += 2900",2)			--给碎银
	     
	   --Hero:ActFun(1046,"34 7",3)					     --马上接取下一个，暂时不用
...
function npcAct3()
	...
	elseif(Hero:ActFun(1080,"task has 0",34) == true)then					--判断任务链是否建立，建立则进入下一步
 	 Hero:ActFun(1080,"step = 7",34)					 --设置此步骤为0
	else
 	 Hero:ActFun(127,"未接受任务",0)					 --显示系统文字
	end
...
if(gContext==3011019)then	
  npcAct1()
elseif(gContext==3011020)then
  npcAct2()
elseif(gContext==3011021)then
  npcAct3()
end
