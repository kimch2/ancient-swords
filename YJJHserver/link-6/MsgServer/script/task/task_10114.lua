...
function npcAct1()
	...
	elseif(Hero:ActFun(1080,"task has 0",2) == true)then				--判断任务链
		...
		if(Hero:ActFun(1080,"step == 2",2) == true)then				--判断任务步骤

		
	  	if(Hero:ActFun(1080,"task state 0",2) == true)then				--判断任务状态

			
			 Hero:ActFun(1080,"task accept 2",2)				--设置任务状态为1
...
function npcAct3()
	...
	elseif(Hero:ActFun(1080,"task has 0",2) == true)then					--判断任务链是否建立，建立则进入下一步
 	 Hero:ActFun(1080,"step = 2",2)					 --设置此步骤为0
 	 Hero:ActFun(498,"0",620000)
	else
 	 Hero:ActFun(127,"未接受任务",0)					 --显示系统文字
	end
